...
async def async_migrate_integration(hass: HomeAssistant) -> None:
    ...
    for entry in entries:
        ...
        if entry.data[CONF_URL] not in url_entries:
            use_existing = True
            all_disabled = all(e.disabled_by is not None for e in entries if e.data[CONF_URL] == entry.data[CONF_URL])
    ...
    if entry.version == 2 and entry.minor_version == 2:
        ...
        for subentry in entry.subentries.values():
            if subentry.subentry_type == "conversation":
                ...
                hass.config_entries.async_update_subentry(entry, subentry, data=MappingProxyType(updated_data))
...
def _add_ai_task_subentry(hass: HomeAssistant, entry: OllamaConfigEntry) -> None:
    ...
    existing_model = next(
        iter(model for subentry in entry.subentries.values() if (model := subentry.data.get(CONF_MODEL)) is not None),
        None,
    )
    if existing_model:
...
def _get_loaded_platforms() -> tuple[Platform, ...]:
    ...
    if find_spec(f"{__package__}.ai_task") is None:
