...
**Directory:** `custom_components/ollama/[platform]/`
...
```python
"""[Platform] platform for Better Ollama."""
...
from .entity import OllamaEntity
from .[entity_file] import Ollama[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: OllamaDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            Ollama[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Better Ollama."""
...
Add platform to `custom_components/ollama/manifest.json`:
...
- [ ] Entity class inherits from both `OllamaEntity` and platform class
...
- **Domain:** `ollama`
- **Class prefix:** `Ollama`
- **Base entity:** `OllamaEntity` in `entity/base.py`
- **Coordinator:** `OllamaDataUpdateCoordinator`
