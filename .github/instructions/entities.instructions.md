...
- **`entity/`** - Base entity classes (inherit `OllamaEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, OllamaEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import OllamaEntity`
