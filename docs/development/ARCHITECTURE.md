...
This document describes the technical architecture of the Better Ollama custom component for Home Assistant.

## Directory Structure

```text
custom_components/ollama/
...
│   ├── __init__.py          # Exports OllamaDataUpdateCoordinator
...
│   ├── __init__.py          # Exports OllamaEntity
...
- `base.py` - Main coordinator class (`OllamaDataUpdateCoordinator`)
...
**Key class:** `OllamaDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `OllamaConfigFlowHandler` (main flow)
- `OllamaOptionsFlow` (options)
...
**Key class:** `OllamaEntity` (in `entity/base.py`)
...
2. `OllamaEntity` for common functionality
...
1. Create directory: `custom_components/ollama/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `OllamaEntity`
...
- Home Assistant 2026.9.0+ - Platform requirements
