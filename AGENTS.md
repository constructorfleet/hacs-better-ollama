...
- **Domain:** `ollama`
- **Title:** Better Ollama
- **Repository:** constructorfleet/hacs-better-ollama

**Key directories:**

- `custom_components/ollama/` - Main integration code
...
- Integration logs: `custom_components.ollama: debug` in `config/configuration.yaml`
...
- Use the domain `ollama` for all DOMAIN references
- Prefix all integration-specific classes with `Ollama`
- Use "Better Ollama" as the display title
...
- Inherit from platform base + `OllamaEntity`
...
- `tests/` mirrors `custom_components/ollama/` structure
